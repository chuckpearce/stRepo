...
metadata {
	...
	tiles {
		standardTile("switch", "device.switch", width: 2, height: 2, canChangeIcon: true) {
			...
			state("offline", label: 'Offline', icon: "st.Outdoor.outdoor12-icn", backgroundColor: "#ff0000", nextState: "off")
		}
		standardTile("modeSwitch", "device.modeSwitch", canChangeIcon: true) {
			state("manual", label: 'Manual', action: "auto", icon: "st.switches.switch.off", backgroundColor: "#e6e035", nextState: "auto")
			state("auto", label: 'Auto', icon: "st.switches.switch.on", backgroundColor: "#79b821", nextState: "manual")
		}
...
def updateDeviceStatus(status) {
	log.debug "Calling status $status"
	if (status == 0) { 
